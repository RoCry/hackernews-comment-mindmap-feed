...
async def main():
    ...
    for idx, story in enumerate(stories):
        ...
        comments_lines = [
            ...
            for line in comment.format_tree(max_length=0)
        ]
        comments_markdown = "\n".join(comments_lines)

        # Generate AI summary
        markdown_summary = await generate_social_comments_markdown_summary(
            story.title, comments_markdown
        )

        # Generate mindmap HTML
        mindmap_html_path = outputs_dir / f"mindmap_{story.id}.html"
        html_content = await generate_mindmap_html(
            markdown_summary, str(mindmap_html_path)
        )
